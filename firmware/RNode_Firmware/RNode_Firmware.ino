...
void button_event(uint8_t event, unsigned long duration) {
    ...
    if (display_blanked) {
      ...
      if (duration > 10000) {
        ...
        // Waypost: new short-hold gesture, doesn't touch the quick-tap
        // BT toggle below or push sleep's threshold up by more than
        // ~600ms. See Display.h's trigger_waypost_logo().
        trigger_waypost_logo();
      } else {
        #if HAS_BLUETOOTH || HAS_BLE
        if (bt_state != BT_STATE_CONNECTED) {
          if (bt_state == BT_STATE_OFF) {
