...
**Active milestone:** **M6** Outpost 🟡 — standalone Outpost firmware (`firmware/outpost`) exists, flashed to real hardware (`/dev/ttyUSB1`); identity persistence bugs found+fixed; Station can now reply to a claimed Outpost (`OUTPOST_CLAIM`, verified over live HTTP). Only remaining gap: the physical Outpost-Wi-Fi→`/claim`→real-LoRa leg needs a human to join the AP by hand — everything upstream is proven. **M4** Identity depth **✅ build-complete** — pairing codes, revocation, `ADMIN_APPROVAL`/admin-role all done; only the Stalwart-vs-SQLite decision remains, a human call not a build task. **M8** Groups/Today/Notice-Beacon polish **✅ done** — Notice ack + Beacon auth (2026-09-23), Today dashboard's two real gaps fixed (2026-09-24), and Groups core + Locker/Dispatch integration (2026-09-24) — all four exit criteria met. **M1b** Pi — software prep done, waiting on SD card (human has it in the mail) — **Station is this dev machine, not a Pi, until then**; **M7** T-Deck also in the mail. **No single obvious next milestone right now** — next hardware-free step is Fieldbook's progressive path, or Beacon propagation through Outposts, or extending Groups scoping to Noticeboard/Commons — see `docs/priority-review.md` §8.  
**Just finished:** M8 slice — Groups core + Locker/Dispatch integration, closing M8 entirely ([docs/groups-and-permissions.md](docs/groups-and-permissions.md)); before that M8 slice — Today/sync dashboard, two real gaps fixed (per-user unread count, cross-app sync total) — third stale "not started" doc claim caught this way in two days ([docs/priority-review.md](docs/priority-review.md)); before that M8 slice — Noticeboard ack + Beacon auth, closing a real spoofing gap in both services' radio paths ([docs/protocol.md](docs/protocol.md), [docs/security.md](docs/security.md)); before that M4 slice 2 — `ADMIN_APPROVAL` + admin-role, portal `/control.html` ([docs/identity.md](docs/identity.md)); before that a full docs refresh (`roadmap.md`, `priority-review.md`, `security.md`) plus committing all of M6 in one commit (`c4f0cc4`); before that M6 `OUTPOST_CLAIM` — Station can reply to a claimed Outpost, reusing the M4 pairing-code system exactly; fixed a matching latent bug in Pocket radio-pairing along the way ([docs/architecture.md](docs/architecture.md)); before that M6 standalone Outpost firmware — real Reticulum via microReticulum (not plaintext), Wi-Fi AP + Corkboard web UI, hardware-verified identity persistence across reboots ([firmware/outpost/README.md](firmware/outpost/README.md)); before that M6 Corkboard — both sides in sim; before that M6 slice 1 — `OutpostNode` LoRa relay (sim) + courier hop-cap/TTL; before that M4 slice 1 — pairing codes + per-device revocation; before that **M2e ✅** over real LoRa
...
**Re:** "Let's keep going, let's get this to beta" → clarified: the human wants the full vision built out iteratively, one slice at a time, indefinitely — not a narrowed "beta" scope — and hardware-gated items get picked up automatically once that hardware arrives. Picked Groups next: the last open M8 item, and (unlike three recent "not started" claims that turned out stale) genuinely had no existing file at all.
**Did:** Planned in Plan Mode first (new subsystem, not a bug fix) — design doc (`docs/groups-and-permissions.md`) had unresolved open questions (custom roles v2?, Outpost key distribution, default-Station-group), so worth deciding v1 scope deliberately rather than building ad hoc. New `server/services/groups/` (`GroupsStore`/`GroupsService`) — `groups` + `group_members(role)` tables, the one authz model other services consult via an injected lookup (same cross-service pattern as `NoticeboardService.get_binding`), never a copy-paste ACL. **v1 enforces exactly two roles** (`member`/`admin`), not the design doc's original five — an explicit, doc-acknowledged v2 deferral (`role` stored as a free string so no schema change needed later). **HTTP/portal-only, no Waylink ops** — checked precedent first: Rollcall has no Waylink registration either, and Groups is a management concern, not something needed mid-hike from a Pocket.
**Integrated with 2 services**, meeting M8's exit criterion ("Groups used by ≥2 services"): **Locker** — `locker_files.group_id` (migration-guarded) + a third `scope` value `'group'`, gated by an injected `is_group_member` lookup, both at upload-time (owner must be a member) and at read-time (viewer must be a member); portal `locker.html` gained a Group scope option and an "A group" view filter. **Dispatch** — `POST /api/dispatch/conversations/rooms` takes an optional `group_id` that seeds the room's membership from the group's *current* members at creation time only — a one-time copy, deliberately not a live link (adding someone to the group afterward doesn't retroactively add them to rooms already created from it — verified this explicitly in a test).
**Portal:** new `groups.html` — create a group (creator becomes its first admin), list mine, click one to see/add/remove members (add/remove admin-only, enforced server-side, hidden client-side for non-admins). New `groups` sidebar nav entry (`shell.js`), own icon.
**Verified:** 12 new tests across `test_groups.py`/`test_locker.py`/`test_dispatch.py` — full suite 145 passed (up from 133), 1 skipped. Live-verified against the running Station: curl with **real per-user sessions** for aj/bob/carol (not the claimed-viewer-query-param trick, which silently no-ops when auth is enforced — `actor_username()` ignores the claimed name and uses the session's own identity; worth remembering for future live checks) confirmed a genuine non-member gets an empty file list and a 404 on download, while a real member sees and downloads it. Also drove the portal UI in a real browser end-to-end (login → Groups page → group with 2 members rendered correctly, admin controls visible). **Heads-up for the human:** during that browser pass the tab visibly flashed between the login screen and the dashboard a few times — that was this agent's own rapid automated login/navigate cycles in the same visible window, not an app bug; it settled once the automation stopped. If it happens again *without* an agent actively driving the browser, that would be worth investigating as a real redirect-loop bug — it wasn't that this time.
**Docs:** `docs/groups-and-permissions.md` (rewritten — v1 shipped, what's deferred to v2), `docs/roadmap.md` (M8 marked ✅, closing progress note), `docs/priority-review.md` (Groups marked done everywhere it was flagged open — §1, §4, §5, §6, §7, §8, §9, §11 — test count updated to 145).
**Next for other agent:** M8 is fully closed. Remaining hardware-free Tier 2 items, all independent: Fieldbook progressive path (still genuinely empty), Beacon propagation through Outposts (cut from the earlier Beacon-auth slice), extending Groups scoping to Noticeboard/Commons (same pattern as Locker, not required for the exit criteria). Per the human's standing direction, keep picking the next one and repeat — hardware-gated items (Pi, T-Deck, MakerHawk) resume automatically once that hardware arrives.
**Blocked:** Nothing.

### 2026-09-24 — Claude (M8 slice: Today/sync dashboard — closed two real gaps, third stale claim caught)

**Re:** "Let's continue" — next pick from the remaining Tier 2 items (Fieldbook, Groups, Today dashboard, Beacon propagation).
**Did:** Checked each candidate against the actual code before picking, same discipline as yesterday's near-miss. Fieldbook genuinely is empty (just a package marker, no service/store/routes/portal — confirmed, not stale this time). `/api/dashboard` turned out to be **substantially built already** — 5-service activity feed, active-Beacon banner, network/online-user status, a "Sync" line — `priority-review.md`'s "not started" label for this was wrong, the **third** stale claim caught in two days (after Postbox, and `ADMIN_APPROVAL`'s real remaining scope).
**Picked Today dashboard, fixed two real gaps instead of rebuilding:**
1. The Noticeboard card showed the **global** active-notice count, not the calling user's own unread count — now uses yesterday's `count_unacked(username)`; label changed "Active Notices" → "Unread Notices" (`web/portal/index.html`).
2. The home page's "Sync" status line only ever reflected Dispatch's pending queue, never Postbox's. New `_sync_status()` helper in `dashboard_routes.py` combines `dispatch.sync_status()`'s `pending_dispatch` with Postbox's `outbox` count into `pending_total` — deliberately **not** the full shared offline-sync subsystem (`priority-review.md` #2, still separately open) — just an honest aggregate of what each app already tracks, no new tracking invented.
**Verified:** 4 new tests (`test_dashboard.py`) — unread-vs-global distinction, per-user independence (two users, two different unread counts), sync aggregate before/after queuing a Postbox outbox item. Full suite 133 passed (up from 131). Confirmed live against the running Station via curl (`sync.pending_mail_outbox`/`pending_total` appear correctly, `noticeboard` card shows `label: "Unread Notices"`). **Browser visual verification hit real, repeated tooling failures this time** — frozen-renderer screenshot timeouts, lost form state across many attempts — stopped after several tries per the "don't rabbit-hole on flaky tools" guidance rather than keep burning cycles; the curl + pytest evidence is solid on its own.
**Docs:** `roadmap.md` (M8 progress), `priority-review.md` (corrected the Today-dashboard and Postbox-adjacent staleness, added an explicit note that every status label in that document is now a hypothesis to verify, not a fact — third time's the pattern).
**Next for other agent:** Groups/permissions core (starts from zero, no existing file — the biggest remaining Tier 2 item) or Beacon propagation through Outposts (deliberately cut from yesterday's auth fix). Both still independent of each other.
**Blocked:** Nothing.

### 2026-09-23 — Claude (M8 slice: Noticeboard ack + Beacon auth — a real spoofing gap fixed)

**Re:** "Go ahead and pick one" — picking from the four independent Tier 2 items the previous entry left open (Postbox, Noticeboard+Beacon, Groups, Today).
**Did:** Before building anything, checked each candidate against the actual code rather than trusting `priority-review.md`'s labels — good thing: **Postbox's "progressive LoRa path" turned out to already be fully built** (`MAIL_STATUS`/`MAIL_LIST`/`MAIL_GET`, compact headers, attachments correctly Wi-Fi-only-metadata, all already wired into the gateway). Would have nearly rebuilt already-finished work. Picked **Noticeboard ack + Beacon auth** instead, and while reading the code to scope it, found the real gap was sharper than "not started": both `NOTICE_CREATE`/`BEACON_PUSH` over Waylink trusted a self-reported `author` field from the packet's own payload — no check against who the radio device actually belongs to. For Beacon specifically (an *emergency alert* system), that meant anyone with a working radio could push, or clear, an alert as anyone.
**Fix:** `NoticeboardService`/`BeaconService` now take an injected `get_binding` lookup (same cross-service pattern `PostboxService.set_nodes_lookup` already used, wired in `main.py` to `db.dispatch.get_binding`) — `NOTICE_CREATE`/`EXPIRE`/`ACK` and `BEACON_PUSH`/`CLEAR` resolve the acting username from the sender's actual device binding, same mechanism Dispatch's `MSG_SYNC` authz already used, and reject unbound senders with `unauthorized_device`. Read-only ops (`LIST`/`GET`) stay open, matching the HTTP routes' existing openness, but annotate with the bound identity's `acked` state when there is one.
**New: Noticeboard ack.** `notice_acks(notice_id, username)` table, `NOTICE_ACK` op, `POST /api/noticeboard/notices/{id}/ack` — idempotent, per-user. Portal `noticeboard.html` gained a "Mark as read" button and an unread count in the status line; unacked notices show **unread** in the sidebar.
**Verified:** 17 new tests (service-level radio-auth rejection/acceptance for both services, ack idempotency, 404 on unknown notice) — full suite 131 passed (up from 122), 1 skipped. HTTP-level verification via a proper persistent cookie jar against the live Station confirmed the real flow (create → `acked: false`, `unacked_count` includes it → ack → `acked: true`, count drops by exactly one). **Browser visual verification hit real tooling flakiness this round** (login/navigation intermittently not completing, one "renderer may be frozen" error) — the curl-based check above proved the live Station itself was working correctly the whole time (it had 2 real active notices while the stuck tab kept showing 0), so this was the extension being unreliable, not a bug in the feature. Didn't keep fighting it once the backend was solidly proven.
**Docs:** `docs/protocol.md` (both service sections), `docs/security.md` (new radio-authz bullet), `docs/roadmap.md` (M8 progress), `docs/priority-review.md` (corrected the stale Postbox claim, marked this item done, added a note-to-self: treat every "not started" line in that document as a hypothesis to verify, not a fact — this is the second stale claim caught this session).
**Next for other agent:** Fieldbook progressive path, Groups/permissions core (starts from zero, no existing file), Today dashboard, or Beacon propagation through Outposts (deliberately cut from this slice — real scope of its own). Still independent, still no single obvious pick.
**Blocked:** Nothing.
